...
export async function fetchKnowledgeBaseMenu(): Promise<string> {
  ...
  try {
    ...
    const documents = await PdfDocument.find({}, 'fileName metadata _id').lean();
    ...
    console.log(`📋 [FETCH MENU]: Menu created with ${documents.length} documents`);
    return menu;
  } catch (error) {
    console.error('❌ [FETCH MENU]: Error fetching knowledge base menu:', error);
...
export async function uploadFileToGeminiForChat(documentId: string): Promise<{ uri: string; mimeType: string }> {
  ...
  try {
    ...
    console.log(`🔎 [UPLOAD]: Retrieved document "${document.fileName}" from MongoDB`);
    ...
    console.log(`✅ [UPLOAD]: Buffer converted successfully. Size: ${fileBuffer.length} bytes`);
    ...
    try {
      ...
      console.log(`📌 [UPLOAD]: File uploaded to Gemini. URI: ${uploadResponse.file.uri}`);
      ...
      while (fileStatus.state === 'PROCESSING' && attempts < maxAttempts) {
        console.log(`⏳ [UPLOAD]: File processing (attempt ${attempts + 1}/${maxAttempts})...`);
      ...
      if (fileStatus.state !== 'ACTIVE') {
        throw new Error(`File failed to process. Final state: ${fileStatus.state}`);
      }
...
export async function generateContextualResponse(
  ...
  console.log('🤖 [GENERATION]: Sending prompt to Gemini with document context...');
...
User Question: ${message}`;
  ...
  try {
    ...
    console.log('✅ [GENERATION]: Response generated successfully with document context');
    return result.response.text();
  } catch (error) {
