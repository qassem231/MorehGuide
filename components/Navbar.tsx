          ...
          <div className="flex items-center gap-3">
            ...
            {user && !isGuest ? (
                ...
                <div className="relative" ref={dropdownRef}>
                  ...
                  >
                    ...
                    <div className="text-right hidden sm:block">
                      ...
                      {/* Role Labels */}
                      {isGuest ? (
                        <p className="text-xs text-amber-600 dark:text-amber-400 font-semibold leading-none">Guest</p>
                      ) : user.email === 'admin@admin.com' || user.isAdmin === true ? (
                        <p className="text-xs text-sky-600 dark:text-sky-400 font-semibold leading-none">Admin</p>
                      ) : user.activeRole === 'student' ? (
                        <p className="text-xs text-emerald-600 dark:text-emerald-400 font-semibold leading-none">Student</p>
                      ) : (
                        <p className="text-xs text-sky-600 dark:text-sky-400 font-semibold leading-none">Lecturer</p>
                      )}
                    </div>
                  ...
                  {isDropdownOpen && (
                    <div className="absolute right-0 mt-2 w-40 sm:w-48 bg-white dark:bg-[#1e1f20] backdrop-blur-sm border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg overflow-hidden animate-in fade-in z-50 transition-colors duration-300">
                      <div className="sm:hidden px-3 py-2 border-b border-gray-200 dark:border-gray-700 text-xs text-gray-600 dark:text-gray-400">
                        ...
                        <p className="text-sky-600 dark:text-sky-400 font-semibold" style={{textTransform: 'capitalize'}}>
              ...
              <div className="hidden sm:flex items-center space-x-4">
                 <Link href="/login" className="text-gray-900 dark:text-gray-100 hover:text-red-600 dark:hover:text-red-400 px-3 py-2 rounded-lg text-sm font-semibold transition-colors">Login</Link>
                 <Link href="/register" className="bg-gradient-brand dark:bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg">Register</Link>
              </div>
            ) : null}
          </div>
        </div>
