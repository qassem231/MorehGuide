...
export default function AdminSetupPage() {
  ...
  const handlePromoteToAdmin = async () => {
    try {
      ...
      user.isAdmin = true;
      // IMPORTANT: Do NOT change the role - keep student/lecturer choice
      ...
      // Redirect and refresh to ensure all components update
      setTimeout(() => {
        router.refresh();
  ...
  return (
    <div className="h-full bg-gradient-brand/5 flex items-center justify-center p-4">
      <div className="bg-brand-dark border border-brand-slate/30 rounded-lg p-8 max-w-md w-full shadow-lg">
        ...
        <p className="text-brand-light mb-6">
          Click the button below to promote your account to admin so you can access the file management dashboard.
        </p>
        ...
        <p className="text-brand-light text-xs mt-4 text-center">
          After promotion, you'll be redirected to the admin file management dashboard.
        </p>
      </div>
    </div>
  );
